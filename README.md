...
Runtime-патч переводит upstream-блок **«Загрузить пример»** и заменяет все штатные англоязычные presets русскими заготовками с подходящими параметрами `exaggeration`, `cfg_weight` и `speed_factor`. Также над textarea добавляется быстрый блок **«Русские шаблоны»**.
...
Выберите preset в **«Загрузить пример»** или в блоке **«Русские шаблоны»**, затем нажмите **«Сгенерировать речь»**. Если upstream UI всё равно вставит английский текст, runtime-патч автоматически заменит его русским стартовым текстом.
